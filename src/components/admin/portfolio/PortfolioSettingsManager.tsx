...
import { reorderPortfolioCategories } from "@/server/actions/portfolio-categories";

export type PortfolioSettingsItem = {
  id: string;
  name: string;
  slug: string;
  coverSrc: string;
...
export function PortfolioSettingsManager({
  initialItems,
}: {
  initialItems: PortfolioSettingsItem[];
}) {
  ...
  async function handleDragEnd(event: DragEndEvent) {
    ...
    try {
      await reorderPortfolioCategories(nextItems.map((item) => item.id));
      setState("saved");
    ...
    <div>
      <div className="mb-4 flex flex-wrap items-center justify-between gap-4">
        <p className="text-xs leading-5 text-[#242617]/45">
          Drag galleries by their handle to change their public display order.
        </p>
