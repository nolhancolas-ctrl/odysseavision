...
export function VideoSettingsManager({
  initialItems,
}: {
  initialItems: VideoSettingsItem[];
}) {
  ...
  async function handleDragEnd(event: DragEndEvent) {
    ...
    try {
      await reorderVideos(nextItems.map((item) => item.id));
      setOrderState("saved");
  ...
  async function changeFeatured(storyId: string) {
    ...
    try {
      await setFeaturedVideo(nextFeaturedId);
      ...
      : orderState === "saved"
        ...
        : orderState === "error"
          ...
          : featuredState === "saving"
            ? "Saving Featured Video..."
            : featuredState === "error"
              ? "Featured Video could not be saved"
    ...
    <div>
      <div className="mb-4 flex items-center justify-between gap-4">
        <p className="text-xs leading-5 text-[#242617]/45">
          Drag videos by their handle. Only one video can be Featured.
        </p>
